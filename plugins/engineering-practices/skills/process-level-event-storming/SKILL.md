...
- **Swimlanes**: happy path along the top; alternative/failure flows stacked below. **External systems sit INLINE in the timeline** — between the command invoked on them and the event they generate (the grammar chain: command → *invoked on* → system → *generates* → event), never in a separate band above the commands; repeat the pink sticky at every interaction it participates in.
